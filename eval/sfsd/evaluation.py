...
from eval.sfsd.eval_utils import HardnessChecker, Schema, tokenize, get_schema, get_tables_with_alias, get_sql
...
class Evaluator:
    ...
    def __init__(self):
        ...
        self.hardness_checker = HardnessChecker()
    ...
    def evaluate(self, gold, predict, db_dir, etype, kmaps, sketch=False):
        with open(gold) as f:
        ...
        for p, g in zip(plist, glist):
            p_str = p[0]
            g_str, db = g
            if 'ref_company_types' in g_str.lower():
                continue
            db_name = db
            ...
            g_sql = get_sql(schema, g_str, sketch=sketch)
            hardness = self.hardness_checker.get_hardness(g_sql)
            scores[hardness]['count'] += 1
            scores['all']['count'] += 1

            try:
                p_sql = get_sql(schema, p_str, sketch=sketch)
            except:
        ...
        self.print_scores(scores, etype)
        return scores
