...
class Preprocessor():
    ...
    def __init__(self, db_content=True):
    ...
    def preprocess_database_annot(self, db: dict, annot: dict, verbose: bool=False):
        """ Tokenize, lemmatize, lowercase table and column names for each database """
        table_toks, table_names = [], []
        for tab in db['table_names']:
            doc = self.nlp(tab)
    ...
    def schema_linking(self, entry: dict, db: dict, verbose: bool = False):
        ...
        if self.db_content:
            if 'cell_values' not in db:
                db_file = os.path.join('data/kaggle-dbqa/databases', db['db_id'], db['db_id'] + '.sqlite')
            ...
            for i, (tab_id, col_name) in enumerate(db['column_names_original']):
                if i == 0 or 'id' in column_toks[i]: # ignore * and special token 'id'
                    continue
                if 'cell_values' in db:
                    cell_values = db['cell_values'][i]
                else:
                    ...
                    try:
                        ...
                        cell_values = [[str(float(each))] if is_number(each) else each.lower().split() for each in
                                       cell_values]
